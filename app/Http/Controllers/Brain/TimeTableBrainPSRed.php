...
class TimeTableBrainPSRed
{
    public static function minimalShapingWithID($id)
    {
        $shape = TimeTableBrainPSRed::shapingWithID ( $id );
        ...
        return TimeTableBrainPSRed::searchSpace ( $start );
    }

    public static function shapingWithID($id)
    {
        $shape = array();
        $time = TimeTableBrainPSRed::betweenTimeWithID ( $id );
        $slots = array();
        $i = 0;
        if ($time == "no") {
            $time = TimeTableBrainPSRed::betweenTime ();
        } else {
            foreach ($time as $t) {
    ...
    public static function betweenTimeWithID($id)
    {
        ...
        foreach ($lecturerBurden as $con) {
            ...
            $m[] = TimeTableBrainPSRed::generateDateRange ( $con->constraints_title , $con->weekday , $con->teacher_id , $start , $end );
        }
        return $m ?? "no";
    }
    ...
    private static function generateDateRange(string $title , string $weekday , string $lectid , Carbon $start_date , Carbon $end_date , $slot_duration = 60)
    {
        $dates = [];
    ...
    private static function betweenTime()
    {
    ...
    public static function searchSpace($events)
    {
        $schedule = [
            'start' => '09:00:00' ,
            'end' => '20:00:00' ,
        ...
        foreach (new DatePeriod( $start , $minInterval , $end ) as $slot) {
            ...
            $jq[] = TimeTableBrainPSRed::slotAvailable ( $slot , $to , $events );
    ...
    private static function slotAvailable($from , $to , $events)
    {
        $wk = [];
        $nwk = [];
        $i = 0;
...
//                        return $this->teachTo ( $cmbl );
                        return TimeTableBrainPSRed::timeAvailDef ( $cmbl );
                    })()));
            } else {
    ...
    public static function shaping()
    {
        $shape = array();
        $time = TimeTableBrainPSRed::betweenTime ();
        $slots = array();
    ...
    protected static function timeAvailDef(Array $p)
    {
        $schedule = [
            'start' => '09:00:00' ,
            'end' => '20:00:00' ,
        ...
        foreach ($p as $people) {
            foreach (new DatePeriod( $start , $minInterval , $end ) as $slot) {
                ...
                $to = $slot->copy ()->add ( $reqInterval );
                $pp = TimeTableBrainPSRed::slotAvailableDef ( $slot , $to , $people );
    ...
    private static function slotAvailableDef($from , $to , $events)
    {
        $wk = [];
        $nwk = [];
        $i = 0;
